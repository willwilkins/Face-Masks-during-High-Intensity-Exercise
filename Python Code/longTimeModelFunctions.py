...
def HRmodel(lamda, L0, HRdata, model_time_vec, HRrest, age, sex, dt=0.1): # should it do it at specific lamdas?
...
def f(lamda, L0, HRdata, timesforHRdata, model_time_vec, HRrest, age, sex, dt=0.1):
    """Cost function for a specified HRdata and HRm datasets, where HRm is the same length as HRdata"""
    
    longHRmodel = HRmodel(lamda, L0, HRdata, model_time_vec, HRrest, age, sex)
    ...
    # the same time
        ...
        # add to f the square of the residual between every corresponding 
        # HRdata and HRmodel element
        for i in range(len(HRdata)):
            # find the time associated with this HRdata[i]
            # ie where timesforHRdata[i] == time vector of HRmodel[j]
            j = np.where(model_time_vec == timesforHRdata[i])
...
def costVector(lamdaVec, HRdata, timesforHRdata, model_time_vec, HRrest, L0, age, sex):
    """ creates a vector of length n of costs across a specified domain of 
    lamda for a HRdataset and model"""

    fvec = np.array([f(lamda, L0, HRdata, timesforHRdata, 
                       model_time_vec, HRrest, age, sex) 
                                                        for lamda in lamdaVec]) 
