...
Functions which work with a higher HRrest
This means the HR0 and time of model start need to be varied and swept across?
...
def HRmodel(lamda, L0, HRdata, timesforHRData, HRrest, age, sex, dt=0.1): # should it do it at specific lamdas?
    ...
    model = np.zeros(int(timesforHRData[-1]/dt + 1)) # multiplier is the number of HRm samples for each HR data sample
    ...
    """# New Method - RK45
    hrdot = deparametriser(HRdot, HRdata[0], HRrest, lamda, age, sex)
    
    model = sp.integrate.RK45(hrdot, 0, HRdata[0], timesforHRData[-1], dt) # with this I can leave dt up to the program
    """
    #hrd = np.array([HRdot(model[i], model[0], lamda, age, sex, dt*i) for i in range(len(model))])
    #return [model, hrd] # not sure if i still need hrd
...
def f(lamda, L0, HRdata, timesforHRdata, HRrest, age, sex, dt=0.1):
    """Cost function for a specified HRdata and HRm datasets, where HRm is the same length as HRdata"""
     
    longHRmodel = HRmodel(lamda, L0, HRdata, timesforHRdata, HRrest, age, sex)
    
    # define HRm as HRmodel from before, sampled at the same time points as the O2Ring
    
    # create a time vector for the case where HRdata hasn't had any values from during the measurement removed
    # (I think this should be a shortcut)
    if timesforHRdata[-1] != 4*(len(timesforHRdata)-1):
        # sum the sqaures of the residuals, assume they're synchronised and HRdata didn't have any data eliminated
            # the final times of HRdata and HRm should be the same, in case HRdata has some missing values
        HRm = np.array([longHRmodel[int(4/dt*i)] for i in range(int(timesforHRdata[-1]/4 + 1))])
        f = np.sum([(HRdata[i] - HRm[i])**2 for i in range(len(HRdata))])
    
    
    # in the case where HRdata has missing values, so timesforHRdata is missing 
    # multiples of 4, the same indices of HRdata and HRm do not correspond to 
    # the same time
    else:
        #print('Working in f, HRdata has missing values.')
        
        #create time vector for the model
        timeVectorforModel = np.arange(0, len(longHRmodel)*dt, dt)
        #print('Length of model\'s time vector:', len(timeVectorforModel))
        #print('check time value of HRmodel is correct')
        #print('Final time value for HRmodel:', timeVectorforModel[-1], 's')
        #print('Final time value for HRdata;', timesforHRdata[-1], 's')
        
        f = 0.
        
        for i in range(len(HRdata)):
            # find the time associated with this HRdata[i]
            # ie where timesforHRdata[i] == time vector of HRmodel[j]
            j = np.where(timeVectorforModel == timesforHRdata[i])
            
            # HR model = HRm, full length
            
            f += (HRdata[i] - longHRmodel[j])**2
    ...
    for i in range(len(HRdata)):
        #j = 4/h*i # so it samples the correct elements of HRm
                    # h = step size of simulation so it s
        f += (HRdata[i] - HRm[i])**2 #sum and square the residuals
    """
    
    return f


# functions which work across lamdas to optimise them
...
def costVector(lamdaVec, HRdata, timesforHRdata, HRrest, L0, age, sex):
    """ creates a vector of length n of costs across a specified domain of lamda for a HRdataset and model"""
        
    fvec = np.array([f(lamda, L0, HRdata, timesforHRdata, HRrest, age, sex) for lamda in lamdaVec]) 
